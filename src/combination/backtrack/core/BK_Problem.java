...
public abstract class BK_Problem extends ArrayList<Variable> {
    ...
    public BK_Problem() {
        super();
    } /// in subclass extend BK_Problem, its constructor need to initialize variable set
    
    public void addVar(Domain D) {
        this.add(new Variable(D));
    } /// support for Initializing variable set
    
    protected void addSolution() {
        Proposal newSol = new Proposal();
        for(Object x : proposal) if(x!=null) newSol.add(x);
    ...
    private void backtrack(int index) {
        if(solutions.size() == nWanted) return;
        int n = this.size();
        ...
        for(Object val : D) {
            ...
            if(accepted(index, val)) {
                v.val = val;
                if(index<n-1) backtrack(index+1);
                else if(accepted()) addSolution();
            }
        }
    }
